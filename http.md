...
+ OSI七层模型 注重通信协议必要的功能是什么
    ...
    - 数据链路层
    - 物理层
+ TCP/IP概念层模型 强调在计算机上实现协议需要开发哪种协议
    - 物理层
    - 链路层
    - 网络层
    - 传输层
...
是一种面向连接的、基于字节流的传输层通信协议。  
在收发前，必须和对方建立可靠的连接。一个tcp连接必须经过三次”对话“才能建立，过程：  
 1. a -> b 我想发送数据，可以吗？ a向b发送数据包
 2. b -> a 可以，什么时候发 b向a发送同意连接和要求同步(两台机器一个接收，一个发送，协调工作)的数据包
 3. a -> b  现在就发，接着吧  a向b发送一个数据包确认b的要求同步  
 三次对话的目的是使数据包的发送和接收同步，经过三次对话后，a才会正式向b发送数据  
 三次握手的特点：  
 没有应用层的数据，syn这个标志只有在tcp建立连接时才会被置为1，握手完成后，syn被置为0  

 tcp建立连接要经过三次握手，断开连接要经过四次挥手  
 1. a成数据传输后，将控制位FIN置为1，提出停止tcp连接的请求
 2. b收到FIN后对其作出响应，确认将这一方向上的tcp连接关闭，将ACK置1
 3. 由b再提出反方向的关闭请求，将FIN置1
 4. a对b的请求进行确认，将ACK置1，双方向的关闭结束  

+ ACK： TCP报头的控制位之一，对数据进行确认。确认由目的端发出，用它来告诉发送端这个序列号之前的数据段都收到了。  
 比如确认号为x，则表示前x-1个数据段都收到了，只有当ACK=1时，确认号才有效；当ACK=0时，确认号无效，这时会要求重传数据，保证数据的完整性。  
+ SYN：同步序列号，TCP建立连接时将这个位置为1
...
### ping
ping命令是使用IP协议和网络控制信息协议ICMP(Internet Control Message Protocol)协议, 是和tcp和udp同一级别  


### TCP/IP
TCP/IP是一个协议簇。里面包含了很多协议。tcp/ip协议本为使用互联网而开发指定的协议。因此，互联网的协议就是tcp/ip协议，tcp/ip协议就是互联网的协议。    
因为tcp、ip协议很重要，因此以这两个命名  
tcp/ip协议集包含了应用层、传输层、网络层  
